...
import { deleteNews, fetchNews } from '../../../store/api-actions/news-api-actions';
...
function NewsScreen(): JSX.Element {
  ...
  const handleSelectAll = (evt: BaseSyntheticEvent) => {
    ...
    setSelectedItems(news.map((newsItem) => newsItem.id));
    ...
    <main>
      <table>
        <caption>
          ...
          <Link to={AdminRoute.NEWS_SHOW}>Добавить новое</Link>
        ...
        <tbody>
          {news.map(({ id, date, title, thumbImage, slug }, i) => (
            <tr key={id} tabIndex={0} onClick={handleItemClick(id)}>
              ...
              <td colSpan={3}>
                <Link to={`${AdminRoute.NEWS_SHOW}?slug=${slug}`} title="Редактировать">
